...
Fetch Linear issue using platform's Linear integration.
...
ELSE:
    Fetch the Linear issue by source_discussion ID
...
3. [PLATFORM: agent-delegation]
   Delegate to Reviewer agent synchronously:
   - Agent: `reviewer`
   - Mode: synchronous (wait for response)
   - Prompt: constructed in steps 1-2 above
...
2. [PLATFORM: agent-delegation]
   Delegate to three review agents in parallel, synchronously:
   - Agents: `code-reuse-reviewer`, `code-quality-reviewer`, `code-efficiency-reviewer`
   - Mode: parallel, synchronous (wait for all responses)
   - Prompts: constructed in step 1 above
