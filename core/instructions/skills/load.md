...
Immediately after Plan Resolution, **delegate to** two Reviewer agents
in parallel, in background if available:

**Reviewer 1: Session State**
- Request: `blueprint hermes request:review:session-state`
- Replace {PLAN_PATH} with resolved plan path

**Reviewer 2: Document Schema** (ADR-007)
- Request: `blueprint hermes request:review:document-schema:session`
- Replace {PLAN_PATH} with resolved plan path

Do NOT wait for either to complete. Proceed immediately to Phase 2.
...
**CRITICAL**:
- Do NOT poll any delegated Reviewer's output
