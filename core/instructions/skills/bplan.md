...
Execute `blueprint lexis --base` to understand the project's base principles.
...
```
blueprint forma copy roadmap {PLANS_DIR}/{nnn}-{topic}/
blueprint forma copy implementation-notes {PLANS_DIR}/{nnn}-{topic}/
```
...
```
blueprint forma copy current-standard {PLANS_DIR}/{nnn}-{topic}/session-context/
blueprint forma copy todo {PLANS_DIR}/{nnn}-{topic}/session-context/
blueprint forma copy history {PLANS_DIR}/{nnn}-{topic}/session-context/
```
...
- [ ] `[PLATFORM: plan-mode]` **Plan Mode entered** (enter plan mode)
