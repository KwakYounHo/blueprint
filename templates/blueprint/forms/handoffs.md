...
[F]Orchestrator&[T]Lorekeeper
---s
```yaml
task:
  action: record | validate
  workflow-id: "{NNN-description}"
  context: "{what to record or validate}"
```
---e

[F]Lorekeeper&[T]Orchestrator
---s
```yaml
handoff:
  status: completed | concerns-found
  summary: "{what was recorded or validated}"
  artifacts:
    - "blueprint/discussions/{NNN}-{summary}.md"
  validation:
    alignment:
      - "{what aligns with discussion}"
    misalignments:
      - "{what doesn't align}"
